...
import { getExams, getPatient } from "@/lib/store";
import { redirect } from "next/navigation";
import { MisDatosSection } from "./MisDatosSection";
...
export default async function MiCuentaPage() {
  ...
  const [allAppointments, myExams, patient] = await Promise.all([
    getAppointments(),
    session.patientId ? getExams(session.patientId) : Promise.resolve([]),
    session.patientId ? getPatient(session.patientId) : Promise.resolve(undefined),
  ]);
      ...
      <main className="mx-auto max-w-3xl px-4 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Consulta tus citas, exámenes médicos y actualiza tus datos.
          </p>
        </div>

        {/* Quick Nav Pills */}
        <nav className="flex items-center gap-2 overflow-x-auto pb-1 text-xs font-semibold">
          <a
            href="#citas"
            className="rounded-full bg-white border border-slate-200 px-3.5 py-1.5 text-slate-700 hover:border-blue-500 hover:text-blue-700 shadow-2xs whitespace-nowrap transition-colors"
          >
            🗓️ Próximas Citas ({upcoming.length})
          </a>
          <a
            href="#resultados"
            className="rounded-full bg-white border border-slate-200 px-3.5 py-1.5 text-slate-700 hover:border-blue-500 hover:text-blue-700 shadow-2xs whitespace-nowrap transition-colors"
          >
            📄 Mis Resultados ({myExams.filter((e) => e.status === "validado").length})
          </a>
          {patient && (
            <a
              href="#mis-datos"
        ...
        <section id="citas" className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden scroll-mt-20">
          ...
          {upcoming.length === 0 ? (
            <div className="px-6 py-12 text-center">
              <p className="text-sm text-slate-400">No tienes citas próximas agendadas.</p>
              <a
                href="/reservar"
        ...
        <section id="resultados" className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden scroll-mt-20">
          <header className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Exámenes y documentos médicos subidos por tu especialista</p>
            </div>
          ...
          {myExams.length === 0 ? (
            <div className="px-6 py-12 text-center">
              ...
              <p className="text-xs text-slate-300 mt-1">Tu terapeuta los subirá y validará después de tus consultas.</p>
            </div>
          ) : (
            <ul className="divide-y divide-slate-100">
              {myExams.map((e) => (
                <li key={e.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="min-w-0 flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {e.notes && (
                        <p className="text-xs text-slate-600 mt-1 italic">{e.notes}</p>
                      )}

                      {e.status === "rechazado" && (
                        <div className="mt-2 rounded-lg bg-red-50 border border-red-200 p-2 text-xs text-red-700">
                          <p className="font-medium">Documento observado por el terapeuta:</p>
                          <p className="mt-0.5">{e.rejectionReason || "Resultado marcado como no válido. Consulta con clínica para mayor información."}</p>
                        </div>
                      )}

                      {e.status === "en_revision" && (
                        <p className="text-2xs text-amber-700 mt-1">
                          Este informe se encuentra en revisión interna por tu terapeuta.
                        </p>
                      )}
                    </div>
                  </div>

                  {/* Download button or unavailable state */}
                  <div className="shrink-0 self-end sm:self-center">
                    {e.status === "validado" ? (
                      <a
                        ...
                        className="rounded-full border border-blue-600 bg-blue-50/50 hover:bg-blue-100/80 px-3.5 py-1.5 text-xs font-semibold text-blue-700 transition-colors flex items-center gap-1.5 shadow-2xs"
                      >
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="rounded-full border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors flex items-center gap-1.5"
                      >
                        Vista previa
                      </a>
                    ) : (
        ...
        {/* Mis Datos Section */}
        {patient && <MisDatosSection patient={patient} />}

        {/* Past appointments */}
        {past.length > 0 && (
          <section id="historial" className="rounded-2xl bg-white border border-slate-200 shadow-sm overflow-hidden scroll-mt-20">
